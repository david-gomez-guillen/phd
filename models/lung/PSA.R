...
zz <- file(paste0(logfilename, Sys.getenv('OMPI_COMM_WORLD_RANK')), open="wt")
#sink(zz)
...
if (length(args)==0) {
  stop('Number of processes missing')
} else {
...
# initEnvir <- function(envir, e) attach(e, name='dompi_envir')
# e <- new.env(parent=emptyenv())
# 
# e$lc.simulate.cpp <- lc.simulate.cpp
# e$lc_simulate_cpp <- lc_simulate_cpp
# e$kStartAge <- kStartAge
# e$kEndAge <- kEndAge
# e$kPeriods <- kPeriods
# e$kHealthy <- kHealthy
# e$kDefaultGroupLength <- kDefaultGroupLength
# e$kOptionsDefaultIncidence <- kOptionsDefaultIncidence
# e$GetAllInterventionsParameters <- GetAllInterventionsParameters
# e$kInterventionsDefaultParameters <- kInterventionsDefaultParameters
# e$age.weights <- age.weights
# 
...
nce=0
# for(par1 in params){    
results <- foreach(par1=params, 
                   .packages=c('doRNG', 'itertools'),
                   .export = c('lc.simulate.cpp', 'lc_simulate_cpp',
                                                                 'kStartAge', 'kEndAge', 'kPeriods', 'kHealthy',
                                                                 'kDefaultGroupLength',
                                                                 'kOptionsDefaultIncidence',
                                                                 'GetAllInterventionsParameters',
...
icer <- unlist(lapply(results, function(e) {e$icer}))
qalyni <- unlist(lapply(results, function(e) {e$qalyni}))
costni <- unlist(lapply(results, function(e) {e$costni}))
qaly <- unlist(lapply(results, function(e) {e$qaly}))
cost <- unlist(lapply(results, function(e) {e$cost}))
ce <- unlist(lapply(results, function(e) {e$ce}))
nce <- unlist(lapply(results, function(e) {e$nce}))
