...
df.params$error <- apply(df.params, 
                         1, 
                         function(r) {
                           as.numeric(r['value']) - df.params[df.params$method=='initial' & df.params$constrained=='yes' &
...
breaks <- c('initial', 'bo', 'nelder-mead', 'annealing', 'pso')
colors <- c('black', '#f8766d', '#619cff', '#00ba38', '#f752df')
...
ggarrange(plt, plt.both, nrow=2, ncol=1, common.legend = TRUE)
```
